...
HoleFillingImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  while (iteration < this->m_MaximumNumberOfIterations)
  {
    ...
    bool                                           hasHole = false;
    itk::ImageRegionConstIterator<OutputImageType> checkIt(this->GetOutput(), this->GetOutput()->GetRequestedRegion());
    ...
    itk::NeighborhoodIterator<OutputImageType> neighbor(radius, snapshot, snapshot->GetRequestedRegion());
    itk::ImageRegionIterator<OutputImageType>  writer(this->GetOutput(), this->GetOutput()->GetRequestedRegion());
