...
def GetSigmaSc(energy, xOverX0, sp, dt):
    ...
    sigmaSc = (
        13.6 * hepunits.MeV / betap * np.sqrt(xOverX0) * (1 + 0.038 * np.log(xOverX0))
    )
...
@click.option(
    "--sp", default=0.15, help="Standard deviation of the tracker uncertainty (mm)"
)
...
@click.option(
    ...
    "--entryTranslation",
    default=0.0,
    help="Amount of translation of the entry detector position in the direction perpendicular to the detector (mm)",
)
@click.option(
    "exitTranslation",
    "--exitTranslation",
    default=0.0,
    help="Amount of translation of the exit detector position in the direction perpendicular to the detector (mm)",
)
def AddTrackerUncertainty(
    xOverX0, sp, dt, input, output, entryTranslation, exitTranslation
):
    dt = dt * hepunits.cm
    sp = sp * hepunits.mm
    ...
    Wentry[:, 0, 0] = 1.0 / np.sqrt(wEntry[:, 0])
    Wentry[:, 1, 1] = 1.0 / np.sqrt(wEntry[:, 1])
    # Wexit = np.diag(1./np.sqrt(wExit[i,:]))
    Wexit = np.zeros((eExit.size, 2, 2))
    Wexit[:, 0, 0] = 1.0 / np.sqrt(wExit[:, 0])
    Wexit[:, 1, 1] = 1.0 / np.sqrt(wExit[:, 1])
