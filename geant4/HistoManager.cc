...
HistoManager::~HistoManager() { delete fHisto; }
...
HistoManager::EndOfRun()
{
  ...
  G4cout << std::setprecision(4) << "Average energy deposit (MeV)         " << fEdepSum / MeV << "   RMS(MeV) "
         << fEdepSum2 / MeV << G4endl;
...
void
HistoManager::ScoreNewTrack(const G4Track * track)
{
  ...
  if (0 == track->GetParentID())
  {
    ...
    if (1 < fVerbose)
      G4cout << "### Primary " << name << " kinE(MeV)= " << e / MeV << "; m(MeV)= " << pd->GetPDGMass() / MeV
             << "; pos(mm)= " << track->GetPosition() / mm << ";  dir= " << track->GetMomentumDirection() << G4endl;
  ...
  {
    if (1 < fVerbose)
      G4cout << "=== Secondary " << name << " kinE(MeV)= " << e / MeV << "; m(MeV)= " << pd->GetPDGMass() / MeV
             << "; pos(mm)= " << track->GetPosition() / mm << ";  dir= " << track->GetMomentumDirection() << G4endl;
    e = std::log10(e / MeV);
    if (pd == G4Gamma::Gamma())
    {
...
void
HistoManager::AddTargetStep(const G4Step * step)
{
  fNstep++;
  G4double fEdep = step->GetTotalEnergyDeposit();
  if (1 < fVerbose)
  {
    G4cout << "TargetSD::ProcessHits: beta1= " << step->GetPreStepPoint()->GetVelocity() / c_light
           << "  beta2= " << step->GetPostStepPoint()->GetVelocity() / c_light
           << " weight= " << step->GetTrack()->GetWeight() << G4endl;
  }
  if (fEdep >= DBL_MIN)
  {
    const G4Track * track = step->GetTrack();

    G4ThreeVector pos = (step->GetPreStepPoint()->GetPosition() + step->GetPostStepPoint()->GetPosition()) * 0.5;
...
void
HistoManager::AddLeakingParticle(const G4Track * track)
{
  const G4ParticleDefinition * pd = track->GetDefinition();
...
void
HistoManager::SetVerbose(G4int val)
{
  fVerbose = val;
...
void
HistoManager::Fill(G4int id, G4double x, G4double w)
{
  fHisto->Fill(id, x, w);
}
